...
const DriverDashboard: React.FC = () => {
  ...
  // 1. Updated Permission Logic - Conditional for Web vs Native
  const handleStartScan = async () => {
    if (isNative) {
      ...
      try {
        ...
        // Add CSS class to make webview transparent (Crucial for Capacitor)
        ...
        if (result.barcodes.length > 0) {
          const id = result.barcodes[0].displayValue.split('/').pop()
  ...
  // 2. Updated Web Scanner Initialization (Only for Web)
  useEffect(() => {
  ...
  return (
    <div className='min-h-screen bg-black text-white flex flex-col relative'>
          ...
          <section className='space-y-4'>
            ...
            {loading ? (
              <div className='p-8 flex items-center justify-center gap-3'>
                ...
                <p className='text-sm text-white/70'>Syncing bookings...</p>
              </div>
            ) : (
              <div className='space-y-3'>
                {bookings.map((booking) => (
                  <Card
                    key={booking.id}
                    className='bg-gradient-to-r from-zinc-950 to-black border-white/10 hover:border-emerald-400/60 transition-all cursor-pointer'
      ...
      {/* --- SCANNER OVERLAY (Conditional UI) --- */}
      {isScanning && (
        <div
          className={`fixed inset-0 z-[60] flex flex-col items-center justify-center p-6 ${!isNative ? 'bg-black/95 backdrop-blur-xl' : 'bg-transparent'}`}
        >
          {/* If Web: Show existing HTML5-QRCode Box. If Native: Body is transparent, just show UI guides */}
          <div className='w-full max-w-md'>
            ...
            <div className='relative p-1.5 bg-gradient-to-tr from-[#9dec75]/40 to-white/5 rounded-[2.5rem]'>
              {!isNative && (
                <div
                  id='qr-reader'
                  className='overflow-hidden rounded-[2.2rem] bg-zinc-950 aspect-square border border-white/10'
                ></div>
              )}
              {isNative && (
                <div className='aspect-square rounded-[2.2rem] border-2 border-dashed border-[#9dec75]/30'></div>
              )}
              {/* UI Corner Guides */}
            ...
            <Button
              ...
              className='w-full mt-10 bg-white/5 text-white/40 hover:text-white rounded-2xl py-6 border border-white/5 transition-colors'
            >
              Cancel
            </Button>
          </div>
      ...
      {scanResult && (
        <div className='fixed inset-0 z-[70] bg-black/90 flex items-center justify-center p-6 backdrop-blur-md'>
          <Card className='w-full max-sm bg-zinc-900 border-zinc-800 text-white'>
            <CardContent className='pt-10 pb-6 text-center'>
              {scanResult.success ? (
                <>
                  ...
                  <p className='text-[#9dec75] font-black text-xl mb-4 uppercase'>
                    {scanResult.data.passengerName}
                  </p>
                ...
                <>
                  ...
                  <p className='text-white/50 px-4 mb-8'>
                    {scanResult.message}
                  </p>
                </>
              )}
              <Button
                ...
                className='w-full bg-white text-black hover:bg-zinc-200 rounded-2xl h-14 font-black uppercase tracking-widest transition-transform active:scale-95'
              >
                Continue
              </Button>
            </CardContent>
        ...
        <div className='fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4'>
          <Card className='bg-gradient-to-b from-zinc-950 to-black border-white/10 text-white w-full max-w-2xl max-h-[90vh] overflow-y-auto'>
            ...
            <CardContent className='pt-6 space-y-6'>
              ...
              <Button
                onClick={() => setSelectedBooking(null)}
                className='w-full rounded-full bg-white text-black font-semibold py-2.5'
              >
                Close
              </Button>
            </CardContent>
