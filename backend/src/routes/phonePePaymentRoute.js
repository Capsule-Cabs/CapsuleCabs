...
const processSuccessfulPayment = async (merchantOrderId, statusData) => {
  const phonePeStatus =
    statusData?.status || statusData?.state || statusData?.data?.state
  ...
  const paymentDoc = await PhonePePayment.findOneAndUpdate(
    ...
    },
    {
      $set: {
        status: mappedStatus,
        transactionId:
          statusData?.transactionId || statusData?.data?.transactionId,
      },
    },
    { new: true },
  ).populate('user')
  ...
  if (mappedStatus === 'SUCCESS' && paymentDoc.pendingBookingData) {
    try {
      ...
      const existing = await Booking.findOne({
        'payment.paymentId': merchantOrderId,
      })
      ...
      if (!existing) {
        ...
        console.log(
          `[SYSTEM] Booking ${newBooking.bookingId} created successfully via payment process.`,
        )
      }
    } catch (error) {
      // Log the error but don't crash the process (payment is already successful)
      console.error(
        '[CRITICAL ERROR] Payment success but Booking failed:',
        error.message,
      )
    }
  }
...
const fetchOAuthToken = asyncHandler(async (req, res) => {
  try {
    ...
    return res.json(ApiResponse.success({ access_token: token }))
  } catch (error) {
...
const createOrder = asyncHandler(async (req, res) => {
  try {
    ...
    if (!amount) {
      return res
        .status(500)
        .json(ApiResponse.error('PhonePe credentials not configured'))
    }

    const orderPayload = {
      merchantOrderId: merchantOrderId,
      amount: Number(Math.round(1 * 100)),
      expireAfter: 1200,
      metaInfo: {
        udf1: phone,
...
const createOrderForSDK = asyncHandler(async (req, res) => {
  try {
    ...
    const base64Request = Buffer.from(JSON.stringify(sdkPayload)).toString(
      'base64',
    )
...
const fetchPaymentStatus = asyncHandler(async (req, res) => {
  ...
  const updatedDoc = await processSuccessfulPayment(
    merchantOrderId,
    response.data,
  )
...
router.get('/', protect, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json(ApiResponse.error('Failed to retrieve payment history'))
  }
})
